...
class DigimonMonitorApp:
    ...
    def initialize_config(self):
        ...
        # Define PT como padrão, já que os textos estão em português por enquanto
    ...
    def apply_language(self):
        """Atualiza o texto de todos os widgets estáticos para o idioma atual, sem precisar reiniciar."""
        t = I18N[self.lang]
        ...
        # Se já existe um save carregado na tela, re-renderiza o log no novo idioma
    ...
    def setup_ui(self):
        ...
        control_frame = tk.Frame(main_frame, bg=PANEL_BG, width=300)
        ...
        # ==========================================
        # SELEÇÃO DE IDIOMA (Bandeirinhas)
        # ==========================================
        ...
        self.pause_frame.pack(pady=30, fill=tk.X, padx=15)
    ...
    def process_save(self, filepath, filename):
        ...
        FARM_START, FARM_END, HEADER_FARM = 0x50000, 0x60000, 0x18
        for offset in range(FARM_START, FARM_END, 4):
            if offset + DIGIMON_SIZE > len(data) or offset in processed: continue
        ...
        # Texto dinâmico vindo do dicionário I18N
        ...
        for loc in ["PARTY", "BOX", "FAZENDA"]:
            ...
            for name, level, talent in alerts[loc]:
                ...
                self.log([
                    ...
                    (f"{name:<18} ({t['lvl_abbr']} {level:02d} / {t['limite_abbr']} {talent:02d}) {txt_atingiu}\n", "alert")
                ])

        if total_almost > 0:
            self.log("-" * 75, "status")
            self.log(txt_quase, "almost")
            self.log("", "status") 
            ...
            for name, level, talent, faltam_int, bar_str, faltam_str, loc in almost_list:
                ...
                self.log([
                    (f" [{loc_lbl[loc]:^7}] ", cor_tag),
                    (f"{name:<14} ({t['lvl_abbr']} {level:02d} / {t['limite_abbr']} {talent:02d}) [{bar_str}] {t['faltam_abbr']} {faltam_str:>7} EXP\n", "status")
                ])
            ...
            if getattr(self, 'show_lv99', False):
                ...
                for loc in ["PARTY", "BOX", "FAZENDA"]:
                    ...
                    for name, level, talent in lv99_list[loc]:
                        ...
                        self.log([
                            (f" [{loc_lbl[loc]:^7}] ", cor_tag),
                            (f"{name:<18} ({t['lvl_abbr']} {level:02d} / {t['limite_abbr']} {talent:02d})\n", "status")
                        ])
                self.log("=" * 75, "header")
